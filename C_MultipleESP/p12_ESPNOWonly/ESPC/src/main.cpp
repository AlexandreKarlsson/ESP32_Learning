...
void sendCommand(int value) {
    ...
    if (!encodeMessage_Command(&stream, ESP_NUMBER, "testCommand", value)) {
        blinkLEDERROR();
        client.stop();
    ...
    while (client.connected() && !received_ACK) {
        if (client.available()) {
            ...
            PayloadData ackPayloadData;
            int ackStatus = 0;

            int decodeResult = decodeMessage(ackBuffer, bytesRead, &ackPayloadData, &ackStatus);

            if (decodeResult == 10) {
                if (ackPayloadData.value == 0) { // Assuming ACK payload represents success
                    received_ACK = true;
                    serverisavailable = true;
                    serverwasavailable = true;
